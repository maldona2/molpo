...
export async function crearFeedback(formData: FormData) {
  ...
  if (formData.get("web")) redirect(`/feedback/${token}/?ok=1`);

  if (rateLimited(token)) {
    redirect(`/feedback/${token}/?error=${encodeURIComponent("Demasiados envíos, probá más tarde")}`);
  }

  const result = validateFeedback(Object.fromEntries(formData.entries()));
  if (!result.ok) redirect(`/feedback/${token}/?error=${encodeURIComponent(result.error)}`);
