...
test("el toast de resuelto sólo si cambió a resuelto y el mail salió", () => {
  assert.equal(avisoTrasMover("abierto", "resuelto", true), "resuelto");
  assert.equal(avisoTrasMover("en_curso", "resuelto", true), "resuelto");
  assert.equal(avisoTrasMover("abierto", "resuelto", false), null);
  assert.equal(avisoTrasMover("resuelto", "resuelto", true), null);
  assert.equal(avisoTrasMover("abierto", "en_curso", true), null);
  assert.equal(avisoTrasMover("resuelto", "cerrado", true), null);
});

test("la URL del tablero lleva el aviso o queda limpia", () => {
  assert.equal(urlTrasMover("resuelto"), "/tablero/?aviso=resuelto");
  assert.equal(urlTrasMover(null), "/tablero/");
});

test("parseAviso sólo acepta avisos conocidos", () => {
  assert.equal(parseAviso("resuelto"), "resuelto");
  assert.equal(parseAviso("otro"), null);
  assert.equal(parseAviso(undefined), null);
});
