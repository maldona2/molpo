...
test("acepta priority null y lo normaliza a medium", async () => {
  const item = {
    id: "r1",
    title: "Sin prioridad",
    description: "desc",
    ...
    url: "https://hub.test/r/r1",
  };
  const pagina = await fetchPagina(cfg, { updatedSince: "x" }, {
    fetch: async () => json({ data: [item], next_cursor: null }),
    esperar: sinEspera,
  });
  assert.equal(pagina.data.length, 1);
  assert.equal(pagina.data[0].priority, "medium");
});
