...
test("mapea shipped/accepted/blocking del Hub", () => {
  assert.deepEqual(mapear(req("s", { status: "shipped", priority: "blocking" }), calla), {
    estado: "resuelto",
    prioridad: "alta",
  });
  assert.deepEqual(mapear(req("a", { status: "accepted", priority: "medium" }), calla), {
    estado: "en_curso",
    prioridad: "media",
  });
});
